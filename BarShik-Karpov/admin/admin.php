...
<body>
    ...
    <div class="content" id="content-tovarfs">
        ...
        <div id="btns-contents">
            ...
            <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#categoriesModal">
                Добавить категорию
            </button>
        ...
        <div id="categories-content">
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) { ?>
                    <a href="?category_id=<?php echo htmlspecialchars($row['category_id']); ?>" class='category-btn' data-category='<?php echo htmlspecialchars($row['name_category']); ?>'><?php echo htmlspecialchars($row['name_category']); ?></a>
            <?php }
            } else {
        ...
        <section id="tovars-control">
            <table>
                ...
                // Проверяем, передан ли ID категории через GET-параметры
                if (isset($_GET['category_id'])) {
                    $categoryId = $_GET['category_id'];
                    // Изменяем SQL-запрос для фильтрации товаров по указанной категории
                    $tovar_control = "SELECT * FROM products WHERE category_id = $categoryId";
                } else {
                ...
                if (mysqli_num_rows($tovar_result) > 0) {
                    while ($tovar = mysqli_fetch_assoc($tovar_result)) {
                        ...
                        if ($tovar['status'] == 'активен') {
                            echo "<td><a href='change-status-tovar.php?id=" . $tovar['id'] . "&status=активен' onclick='confirmRemove()' class='remove'>Скрыть</a></td>";
                        } else {
                        ...
                        echo "<td><a href='delete-tovar.php?id=" . $tovar['id'] . "' onclick='return confirmDelete()' class='delete'>Удалить</a></td>";
                        echo "</tr>";
                    }
                } else {
