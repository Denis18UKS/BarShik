...
if(mb_strlen($password) < 5 || mb_strlen($password) > 100){//mb_strlen — Получает длину строки
	echo "<script>alert('Недопустимая длина пароля!' \n 'Минимальная длина пароля 6 символов'); location.href='reg.php';</script>";
	exit();
}
...
if(!empty($user1)){//empty — Проверяет, пуста ли переменная
	echo "<script>alert('Данная почта уже используется!'); location.href='reg.php';</script>";
	exit();
}
if(!empty($user2)){//empty — Проверяет, пуста ли переменная
	echo "Данный (а что данный то?) уже используется!";
	exit();
}
